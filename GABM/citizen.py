...
class Citizen(mesa.Agent):
    ...
    #改进prompt-添加经济状态，因为出门工作与否其实是与自己的经济状态联系紧密的

    #方案一：非量化
    # 添加一行属性
    #financial state: constrained,moderate,well-off
    ...
    def get_response_and_reasoning(self):
        '''
        GAI of model. Uses LLM to provide response and reasoning to the prompt provided.
        '''
        #给出系统消息，为模型设置一些全局的指导性信息，以影响模型在对话中的行为
        system_prompt = f"""
        ...
        Based on 1.your traits 2.health condition 3.daily infection rate in the town 4. work need, should {self.name} stay at home for the entire day? Please provide your reasoning.
        please pay atention!
        "Yes" means stay at home,"No" means not stay at home but go to work instead,your "Response" must align with "Reasoning". This is crucial.
